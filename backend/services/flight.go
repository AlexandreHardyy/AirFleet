...
//func (s *FlightService) SimulateStartFlight(flightId int) error {
//	flight, err := s.repository.GetFlightByID(flightId)
//	if err != nil {
//		return err
//	}
...
//		return errors.New("flight is not in the correct status to start")
//	}
//
//	flight.Status = flightStatus.IN_PROGRESS
//	//err = s.repository.UpdateFlight(flight)
//	_, err = s.repository.UpdateFlight(flight)
//	if err != nil {
//		return err
//	}
//
//	go s.simulateFlightProgress(flightId)
//
//	return nil
//}
//
//func (s *FlightService) simulateFlightProgress(flightId int) {
//	ticker := time.NewTicker(5 * time.Second) // Exemple : mise à jour toutes les 5 secondes
//	defer ticker.Stop()
//
//	for {
//		select {
//		case <-ticker.C:
//			// Mettre à jour la position de l'avion ici
//			// Ceci est un exemple, vous devrez adapter selon votre modèle de données
//			flight, err := s.repository.GetFlightByID(flightId)
//			if err != nil || flight.Status != flightStatus.IN_PROGRESS {
//				return // Arrêter la goroutine si le vol est terminé ou en erreur
//			}
//
//			// Simuler le déplacement de l'avion
//			//flight.Latitude += 0.01
//			//flight.Position.Longitude += 0.01
//
//			// Vérifier si le vol a atteint sa destination
//			// if flight.Position.Latitude >= destination.Latitude && flight.Position.Longitude >= destination.Longitude {
//			//     flight.Status = flightStatus.COMPLETED
//			//     s.repository.UpdateFlight(flight)
//			//     return // Arrêter la goroutine si le vol est terminé
//			// }
//
//			// Mettre à jour la position et le statut du vol dans la base de données
//			// err = s.repository.UpdateFlight(flight)
//			// if err != nil {
//			//     log.Println("Failed to update flight:", err)
//			//     return
//			// }
//		}
//	}
//}
//
//func (s *FlightService) StartAndCompleteFlight(flightId int, pilotId int) error {
//	// Déclencher le décollage
//	err := s.FlightTakeoff(flightId, pilotId)
//	if err != nil {
//		return err
//	}
//
//	// Récupérer la destination de l'avion
//	flight, err := s.repository.GetFlightByID(flightId)
//	if err != nil {
//		return err
//	}
...
//				FlightId:  flightId,
//				Latitude:  flight.DepartureLatitude + 0.01,
//				Longitude: flight.DepartureLongitude + 0.01,
//			}
//
//			_, err := s.PilotPositionUpdate(newPosition, pilotId)
//			if err != nil {
//				log.Println("Erreur lors de la mise à jour de la position:", err)
//				return
//			}
//
//			// Vérifier si l'avion a atteint sa destination
//			// Cette vérification est simplifiée, elle doit être remplacée par une logique appropriée
//			if newPosition.Latitude >= flight.ArrivalLatitude && newPosition.Longitude >= flight.ArrivalLongitude {
//				break // Sortir de la boucle pour atterrir
//			}
//		}
//
//		// Atterrissage de l'avion
//		err = s.FlightLanding(flightId, pilotId)
//		if err != nil {
//			log.Println("Erreur lors de l'atterrissage:", err)
//		}
//	}()
//
//	return nil
